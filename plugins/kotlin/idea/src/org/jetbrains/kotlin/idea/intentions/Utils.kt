...
fun KtExpression.isBooleanPrefixExpression(): Boolean {
    val baseExpression = (this as? KtPrefixExpression)?.baseExpression ?: return false
    val bindingContext = baseExpression.analyze(BodyResolveMode.PARTIAL)
    val type = bindingContext.getType(baseExpression) ?: return false
    return KotlinBuiltIns.isBoolean(type)
}

fun KtExpression.negate(reformat: Boolean = true): KtExpression {
    return negate(reformat) { it.isBooleanPrefixExpression() }
}
